...
async def create_new_pack(update: Update, context: ContextTypes.DEFAULT_TYPE, state: dict):
    ...
    try:
        ...
        with open(placeholder, "rb") as f:
            sticker = InputSticker(sticker=f, emoji_list=["\U0001f3ac"], format="video")
...
async def video_note_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            ok = convert(inp, out)
            if not ok:
                await msg.edit_text("Не удалось конвертировать. Попробуй кружок покороче.")
                return
        ...
        for crf in [30, 40, 50, 58]:
            r = subprocess.run(
                [
                    ...
                    "-b:v", "0", "-crf", str(crf),
                    "-auto-alt-ref", "0",
                    "-an", "-deadline", "realtime", "-cpu-used", "8",
                    out,
                ],
